const CCAdmin = (() => {
  ...
  function report() {
    ...
    if (!from || !to || !Number.isFinite(duration) || duration < 0 || duration > 366 * 86400000) {
    ...
    panel.querySelector("p.muted").textContent = all
      ? "Theo ngày đặt đơn, gồm đơn chưa thanh toán. Đây là lãi dự kiến, chưa trừ chi phí vận hành."
      : "Theo ngày đặt đơn, chỉ tính đơn đã thanh toán. Lãi gộp chưa trừ chi phí vận hành.";
    ...
    function fail(error) {
      ...
      if (code.includes("permission-denied")) {
        message = "Firebase từ chối quyền đọc đơn hàng. Hãy triển khai firestore.rules đúng dự án, rồi đăng xuất và đăng nhập lại Admin.";
      } else if (code.includes("unauthenticated")) {
        message = "Phiên đăng nhập hết hạn. Vui lòng đăng nhập lại Admin.";
      } else if (code.includes("unavailable")) {
        message = "Chưa kết nối được Firebase. Kiểm tra mạng và bấm Xem báo cáo để thử lại.";
      }
      el("profitCards").innerHTML = "";
      el("profitRows").innerHTML = '<tr><td colspan="6">Không tải được dữ liệu.</td></tr>';
      ...
      reportOff = db.collection("orders")
        ...
        .onSnapshot(snapshot => {
          try {
            ...
            const empty = () => ({count: 0, revenue: 0, cost: 0, knownRevenue: 0, missing: 0, discount: 0});
            ...
            const cards = [
              ...
              [all ? "Lãi gộp dự kiến đã biết" : "Lãi gộp đã biết", covered ? cash(profit) : "—"],
            ...
            el("profitCards").innerHTML = cards.map(([label, value]) =>
              `<article class="profit-card"><span>${label}</span><strong>${value}</strong></article>`
            ).join("");

            let message = snapshot.metadata?.fromCache ? "Dữ liệu lưu tạm; đang chờ đồng bộ Firebase. " : "";
            if (!orders.length) message += "Không có đơn trong khoảng ngày đã chọn. Thử mở rộng khoảng ngày.";
            else if (!selected.length) message += "Có " + orders.length + " đơn nhưng chưa có đơn đã thanh toán. Chọn Tất cả đơn để xem lãi dự kiến.";
            else if (sum.missing) message += sum.missing + " đơn thiếu giá vốn bị loại khỏi phép tính lãi và tỷ suất; vẫn được tính trong giá trị đơn.";
            else message += "Đã hiển thị " + selected.length + " đơn. Giá vốn lấy tại thời điểm đặt đơn.";
            ...
            el("profitRows").innerHTML = [...rows]
              ...
              .map(([date, r]) => `<tr>
                ...
                <td>${r.count > r.missing ? cash(r.knownRevenue - r.cost) : "—"}</td>
                <td>${r.missing}</td></tr>`).join("")
              || '<tr><td colspan="6">Không có đơn phù hợp với bộ lọc.</td></tr>';
